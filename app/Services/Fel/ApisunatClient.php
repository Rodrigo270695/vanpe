...
final class ApisunatClient
{
    ...
    /**
     * @param  array{tipo_doc: int, num_doc: string, nombre: string, direccion: string}  $receptor
     * @return array<string, mixed>
     */
    public function construirPayload(
        ...
        $igvPct = number_format((float) $settings->tax_rate, 0, '.', '');
        $timezone = app(TenantManager::class)->tenant()?->timezone ?: 'America/Lima';
        $fecha = now($timezone);

        $items = $sale->items->map(function (SaleItem $item) use ($igvPct): array {
            ...
            return [
                ...
                'descripcion' => mb_substr($item->nombre_snapshot, 0, 250),
